...
p1 = plot(title=curr_title,
    ...
    ylims=(minimum(uₙ[:,1]), maximum(uₙ[:,1])),
    minorgrid=true
)
...
for i in 1:N
    plot!(p1, t, uₙ[:,i], ls=:solid, label="")
end

p = plot(p1, legend=false, size=(800,500), dpi=300, left_margin=(20,:px))

savedir = plotsdir(GENERAL_PLOT_SAVE_DIR, "09-(t,un)-loop_integrate")
savename = "09-$(approx_type)-(t,un)-loop_integrate_$(time_ns()).pdf"
