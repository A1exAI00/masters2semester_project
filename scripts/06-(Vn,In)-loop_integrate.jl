...
p1 = plot(title=curr_title,
    titlefontsize=10,
    xlabel="Vₙ, В", ylabel="Iₙ, А", 
    label=nothing,
    xlims=(minimum(Vₙ[:,1]), maximum(Vₙ[:,1])),
    ylims=(minimum(Iₙ[:,1]), maximum(Iₙ[:,1])),
    minorgrid=true
)
...
for i in 1:N
    plot!(p1, Vₙ[:,i], Iₙ[:,i], ls=:solid, label="")
end

p = plot(p1, legend=false, size=(800,500), dpi=300, left_margin=(20,:px))

savedir = plotsdir(GENERAL_PLOT_SAVE_DIR, "06-(Vn,In)-loop_integrate")
savename = "06-$(approx_type)-(Vn,In)-loop_integrate_$(time_ns()).pdf"
