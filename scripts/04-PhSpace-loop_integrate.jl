...
plot_xlabel = if dimentional "Vₙ" else "uₙ" end
...
p1 = plot(title=plot_title,
    ...
    ylims=(minimum(Y[:,N+1]), maximum(Y[:,N+1])),
    minorgrid=true
)
...
for i in 1:N
    plot!(p1, Y[:,0N+i], Y[:,1N+i], ls=:solid, label="")
end

p = plot(p1, legend=false, size=(800,500), dpi=300, left_margin=(20,:px))

savedir = plotsdir(GENERAL_PLOT_SAVE_DIR, "04-PhSpace_loop_integrate")
savename = "04-$(approx_type)-$(dimentional)-PhSpace_loop_integrate_$(time_ns()).pdf"
